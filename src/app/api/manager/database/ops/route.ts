...
function getDatabaseProvider(schemaPath: string): string {
  try {
    if (!fs.existsSync(schemaPath)) return "postgresql";
    ...
    if (datasourceMatch) {
      ...
      const providerMatch = datasourceBody.match(/provider\s*=\s*"([^"]+)"/);
      if (providerMatch) {
...
function ensureMigrationLock(projectPath: string) {
  ...
  const provider = getDatabaseProvider(schemaPath);
  const expectedContent = `# Please do not edit this file manually\n# It should be added to your version control system\n# More information: https://pris.ly/d/migration-engine-lock\n\nprovider = "${provider}"\n`;
  
  const lockPath = path.join(migrationsDir, "migration_lock.toml");
  let needsWrite = true;
  if (fs.existsSync(lockPath)) {
    try {
      const existingContent = fs.readFileSync(lockPath, "utf-8");
      if (existingContent.includes(`provider = "${provider}"`)) {
        needsWrite = false;
      }
    } catch {
      // Overwrite if reading fails
    }
  }
  
  if (needsWrite) {
    fs.writeFileSync(lockPath, expectedContent, "utf-8");
  }
}
