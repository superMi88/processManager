...
// Attempt to connect to PM2 with a 3-second timeout protection
function connectPm2(): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      if (active) {
        active = false;
        reject(new Error("PM2 connection timed out after 3000ms"));
      }
    }, 3000);
...
function runPm2<T>(action: () => Promise<T>, fallback: () => T): Promise<T> {
  if (isMockMode) {
...
export async function restartProcess(nameOrId: string | number): Promise<boolean> {
  ...
  return runPm2(
    () => new Promise((resolve) => {
      ...
      (pm2 as any).restart(nameOrId, (err: any) => {
        if (err) {
